...
def plot_out(dir_path, option):            
    ...
    maps['range0'] = maps['range0'].apply(lambda x: int(x, 16)/4096)
    maps['range1'] = maps['range1'].apply(lambda x: int(x, 16)/4096)
    ...
    for name, region in rsyslog.groupby("axis"):
        ...
        #axes[converty].set_ylim(min(LINE[0]-PADDING, REGION_START-PADDING), REGION_END+PADDING)
        #axes[converty].hlines( y=LINE[0], xmin=0, xmax=40*SEC_TO_USEC, colors='red', linestyles='solid')
        #axes[converty].hlines( y=LINE[1], xmin=0, xmax=40*SEC_TO_USEC, colors='blue', linestyles='solid')
